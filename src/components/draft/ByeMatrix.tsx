...
export function ByeMatrix({ players, layout = "row" }: { players: Player[]; layout?: "row" | "column" }) {
  ...
  return (
    <section className={cn("pb-6", column ? "px-0" : "px-3")}>
          ...
          {weeks.map(({ week, players: list, conflict }) => (
            ...
            >
              ...
              <ul className="space-y-1">
                ...
                {list.length === 0 && <li className="text-center text-[10px] text-muted-foreground">—</li>}
              </ul>
            </div>
          ))}
        </div>
      ...
      {unknown.length > 0 && (
        <p className="mt-2 text-[11px] text-muted-foreground">Bye unknown: {unknown.map((p) => p.name).join(", ")}</p>
      )}
    </section>
  );
}
