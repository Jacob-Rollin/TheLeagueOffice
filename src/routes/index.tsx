...
function Home() {
  ...
  useEffect(() => {
    fetch(NEWS_URL).then((r) => r.ok ? r.json() : Promise.reject()).then((d) => setNews((d.articles ?? []).slice(0, 6))).catch(() => setNews([]));
    try {
  ...
  return <main className="mx-auto w-full max-w-6xl px-3 pb-16">
    ...
    <section className="mt-10"><div className="mb-3 flex items-end justify-between"><div><p className="eyebrow">Around The League</p><h2 className="display-title text-3xl">Recent NFL News</h2></div><span className="text-[10px] uppercase tracking-widest text-muted-foreground">Updated on load</span></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{news.length ? news.map((n, i) => <a key={`${n.headline}-${i}`} href={n.links?.[0]?.web?.href ?? "#"} target="_blank" rel="noreferrer" className="group overflow-hidden rounded-xl border border-border bg-card hover:border-primary">{n.images?.[0]?.url && <img src={n.images[0].url} alt={n.images[0].alt ?? "NFL news"} className="h-32 w-full object-cover"/>}<div className="p-4"><p className="text-[10px] uppercase tracking-widest text-primary">NFL News</p><h3 className="mt-1 font-semibold leading-5 group-hover:text-primary">{n.headline}</h3>{n.description && <p className="mt-2 line-clamp-2 text-xs text-muted-foreground">{n.description}</p>}</div></a>) : ["Training camp notes to watch","Preseason depth-chart battles","Fantasy players trending up"].map((x) => <div key={x} className="rounded-xl border border-border bg-card p-4"><p className="text-[10px] uppercase tracking-widest text-primary">NFL News</p><h3 className="mt-1 font-semibold">{x}</h3><p className="mt-2 text-xs text-muted-foreground">Live NFL headlines will appear here when the news feed is available.</p></div>)}</div></section>
  </main>;
}
