...
export const Route = createFileRoute("/hof")({
  ...
  notFoundComponent: () => (
    <div className="min-h-screen bg-zinc-950 py-24 text-center text-zinc-400">
...
function HofError({ error }: { error: Error }) {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 py-24 text-center" role="alert">
    ...
    <main className="min-h-screen bg-zinc-950 p-4 text-white md:p-8">
      <div className="mx-auto max-w-4xl">
        <header className="py-8 text-center md:py-12">
          ...
          <h1 className="mt-2 bg-gradient-to-b from-amber-200 to-yellow-600 bg-clip-text font-display text-4xl font-black uppercase tracking-tight text-transparent sm:text-5xl md:text-6xl">
            Hall of Fame
          </h1>
        ...
        {years.length === 0 ? (
          ...
          <div className="relative pb-16">
            ...
            <div
              aria-hidden
              className="absolute left-1/2 top-0 bottom-0 z-0 w-1 -translate-x-1/2 bg-gradient-to-b from-amber-500 to-yellow-600"
            />

            <div className="space-y-12 md:space-y-16">
              {years.map((entry, i) => (
                <YearNode key={entry.year} entry={entry} index={i} />
              ))}
            </div>
          </div>
        )}
...
function YearNode({ entry, index }: { entry: HofYear; index: number }) {
  const shiftLeft = index % 2 === 0;

  return (
    <section className="relative">
      {/* Year milestone chip centered on the timeline */}
      <div className="relative z-10 mx-auto my-8 w-max rounded-full border border-amber-500/30 bg-zinc-900 px-6 py-2 text-2xl font-extrabold text-amber-400 shadow-md shadow-amber-500/10">
        {entry.year}
      </div>

      {/* Row 1: Championship card — large and alternating */}
      <div
        className={cn(
          "mx-auto w-full max-w-2xl transition-transform duration-300 md:duration-0",
          shiftLeft ? "md:-translate-x-6" : "md:translate-x-6",
        )}
      >
        <FlipCard
          className="h-80 w-full md:h-96"
          front={
            <div className="flex h-full flex-col items-center justify-center gap-4 overflow-hidden rounded-xl border border-amber-500/30 bg-gradient-to-br from-zinc-900 to-zinc-950 p-6 text-center shadow-[0_0_40px_-18px_rgba(251,191,36,0.7)]">
              <TrophyIcon />
              <span className="bg-gradient-to-b from-amber-200 to-yellow-600 bg-clip-text font-display text-5xl font-black leading-none tracking-tighter text-transparent sm:text-6xl md:text-7xl">
                {entry.year}
              </span>
      ...
      {/* Row 2: Three smaller record cards side-by-side */}
          ...
          back={
            <CardBack
              ...
              rows={[
                ...
                ["Week", entry.playerWeek?.week ? `Week ${entry.playerWeek.week}` : undefined],
          ...
          back={
            <CardBack
              ...
              rows={[
                ...
                ["Week", entry.teamWeek?.week ? `Week ${entry.teamWeek.week}` : undefined],
  ...
  return (
    <div className={cn("[perspective:1200px]", className)}>
      <button
        ...
        className="relative h-full w-full cursor-pointer overflow-hidden rounded-xl text-left transition-all duration-300 [transform-style:preserve-3d] hover:scale-[1.03] hover:shadow-[0_0_35px_rgba(245,158,11,0.6)] focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-400"
...
function SmallFront({ label }: { label: string }) {
  return (
    <div className="flex h-full flex-col items-center justify-center gap-2 overflow-hidden rounded-xl border border-zinc-800 bg-zinc-900/60 p-6 text-center transition-colors hover:border-amber-500/40">
      <span className="h-px w-8 bg-gradient-to-r from-transparent via-amber-400 to-transparent" />
      <span className="font-display text-sm font-semibold uppercase leading-snug tracking-wide text-zinc-200 md:text-base">
        {label}
      </span>
  ...
  return (
    <div
      className={cn(
        ...
        large
          ? "rounded-xl border-amber-500/40 shadow-[0_0_40px_-18px_rgba(251,191,36,0.7)]"
          : "border-zinc-800",
      )}
    >
      ...
      {rows.map(([label, value]) => (
        <p
          key={label}
          className={cn(
            "max-w-full text-wrap break-words text-zinc-300",
...
function TrophyIcon() {
  return (
    <svg
      viewBox="0 0 24 24"
      aria-hidden
      className="h-10 w-10 text-amber-400 drop-shadow-[0_0_10px_rgba(251,191,36,0.6)] md:h-12 md:w-12"
      fill="currentColor"
    >
