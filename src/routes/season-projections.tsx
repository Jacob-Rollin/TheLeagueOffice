...
function SeasonProjectionsPage() {
  ...
  const rows = useMemo((): EnrichedSeasonRow[] => {
    ...
    const enriched = players
      ...
      .map((p) => {
        ...
        const proj = scoreResearchProjection(
          stats,
          scoringFormat,
          scoringOverride,
          scoringMap,
  ...
  const seasonLabel = season ?? String(new Date().getFullYear());
  ...
  return (
    <main className="mx-auto w-full max-w-shell px-3 pb-16 pt-6">
      ...
      <div className="mb-4 rounded-xl border border-sky-100 bg-sky-50/80 px-4 py-3 text-sm text-slate-600">
        <p className="font-semibold text-slate-800">How season projections are calculated</p>
        <p className="mt-1 leading-relaxed">
          We pull Sleeper’s full-season projection line (pass/rush/rec, etc.) and convert it to
          fantasy points with the scoring system you select above
          {hasLeague
            ? " — defaulting to your synced league’s rules until you switch presets"
            : ""}
          . Sleeper publishes season and weekly projections separately, so adding up every week in
          Sleeper will not match this season total.
        </p>
      </div>
...
const SeasonProjRow = memo(function SeasonProjRow({
  row,
  rank,
  statCols,
  onOpen,
}: {
  row: EnrichedSeasonRow;
  rank: number;
