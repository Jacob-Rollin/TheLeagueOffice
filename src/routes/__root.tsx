...
const NAV: [string, string][] = [["/", "League HQ"], ["/draft", "War Room"], ["/trade", "Trade Evaluator"], ["/waiver", "Waiver Wire"]];
function SiteNav() { return <header className="border-b border-border bg-background/90 backdrop-blur"><nav className="mx-auto flex w-full max-w-6xl items-center gap-2 px-3 py-2.5"><Link to="/" className="display-title mr-2 whitespace-nowrap text-lg">THE LEAGUE <span className="text-primary">OFFICE</span></Link><div className="flex flex-1 gap-1 overflow-x-auto">{NAV.map(([to, label]) => <Link key={to} to={to} activeOptions={{ exact: to === "/" }} className="rounded-md border border-transparent px-3 py-1.5 font-display text-sm uppercase tracking-wide text-muted-foreground transition-colors hover:text-foreground data-[status=active]:border-primary data-[status=active]:bg-primary data-[status=active]:text-primary-foreground">{label}</Link>)}</div></nav></header>; }
function RootComponent() { const { queryClient } = Route.useRouteContext(); return <QueryClientProvider client={queryClient}><SiteNav /><Outlet /></QueryClientProvider>; }
